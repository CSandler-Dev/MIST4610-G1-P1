...
The **Membership_Plans** entity defines the different types of memberships offered by the club. Each plan includes attributes such as membership type, cost, and duration. The one-to-many relationship between **Membership_Plans** and **Players** Memberships ensures that multiple players can subscribe to the same plan each player's membership is linked to only one plan at a time. But the **Player_Memberships** entity acting as the bridge between players and their chosen membership plans. It records which membership plan a player is enrolled in, along with the start and end dates of their membership.
